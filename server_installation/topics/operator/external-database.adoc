...
For more information on encoding, see the https://kubernetes.io/docs/concepts/configuration/secret/[Kubernetes Secrets manual].
endif::[]
...
The Keycloak custom resource also needs to be updated to enable external database support. Here's an example:

.Example YAML file for `Keycloak` custom resource that uses an external database
```yaml
...
. Locate the secret for your PostgreSQL database using this command: `oc get secret <secret_for_db> -o yaml`. For example:
+
...
----
+
`POSTGRES_EXTERNAL_ADDRESS` appears in Base64 format.

. Decode the value for the secret using this command: `echo "<encoded_secret>" | base64 -decode`. For example:
+
[source,bash,subs=+attributes]
----
...
----
+
The `keycloak-postgresql` service sends requests to a set of IP addresses in the cluster for your database.  These IP addresses are called endpoints.
